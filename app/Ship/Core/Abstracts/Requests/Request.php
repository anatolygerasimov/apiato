...
abstract class Request extends LaravelRequest
{
    ...
    {
        return App::make(GetAuthenticatedUserTask::class)->run()->id === $userId;
    }
     ...
     * @return static
     */
    public static function injectData(array $parameters = [], ?User $user = null, array $cookies = [], array $files = [], array $server = [])
    {
     ...
     * For example, ['data.attributes.name' => 'firstname'] would map the field [data][attributes][name] to
     * [firstname].
     * Note that the old value (data.attributes.name) is removed the original request - this method manipulates the
     * request! Be sure you know what you do!
     *
     * @param array $fields
     */
