...
trait TestsRequestHelperTrait
{
     ...
     * @throws UndefinedMethodException
     */
    public function makeUploadCall(array $data = [], array $headers = [])
    {
        // Get or create a testing user. It will get your existing user if you already called this function from your
        // test. Or create one if you never called this function from your tests "Only if the endpoint is protected".
        $this->getTestingUser();
        ...
        $httpResponse = $response = $this->call($verb, $url, $params, $cookies, $data, $server);
    ...
    {
        return config('apiato.api.url') . Apiato::getApiPrefix() . ltrim($uri, '/');
    }
    ...
    {
        return (config('apiato.hash-id') && !$skipEncoding) ? Hashids::encode($id) : $id;
    }
    ...
    {
        $responseLoggerEnabled = config('debugger.tests.response_logger');
